...
const removeFromHistory=async(dispatch,id)=>{
    try{
        const {data:{response},status}=await axios.delete(`https://video-library-server.vineetht.repl.co/history/${id}`)
        if(status===200){
            dispatch({type:'SET_HISTORY',payload:response})
        }else{
            dispatch({type:'TOAST',payload:'Server Error'})
        }  
    }
    catch(error){
        console.log(error)
          ...
          const{id}=video
                    return (
                            <div className='playlist-videos'>
                                ...
                                <button className='btn-primary' onClick={()=>removeFromHistory(dispatch,id)}>
