...
export async function findCurrentVideo(
  setVideoPlayingNow,
  videoId,
  dispatch,
  navigate
  ...
  const {
    data: { response, message, videoPlaying }
  } = await axios.get(`${API}/videos/${videoId}`);
  if (response) {
...
export async function likeUnlikeVideo(videoId, dispatch) {
  try {
    const {
      data: { response, liked, message }
    } = await axios.post(`${API}/likedVideos`, { videoId });
    if (response) {
...
export async function getLikedVideosFromDB(dispatch) {
  try {
    const {
      data: { response, liked, message }
    } = await axios.get(`${API}/likedVideos`);
    if (response) {
...
export function expectionHandlerForServiceCalls(
  logoutHandler,
  navigate,
  authDispatch
) {
