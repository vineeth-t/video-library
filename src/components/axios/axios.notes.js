...
export async function getNotesFromDB(dispatch,videoId){
    try{
      const {data:{response,notesFolder:{notes}}}=await axios.get(`${API}/notes/${videoId}`)
  ...
  export async function notesHandler(e,videoId,note,dispatch,flag){
    ...
    const {data:{response,notesFolder:{notes}}}=await axios.post(`${API}/notes/${videoId}`,{note,flag})
    console.log(response,notes)
    if(response){
      dispatch({type:'SET_NOTES',payload:notes})
    } 
  ...
  export async function deleteNotesByIndex(videoId,notesIndex,dispatch){
    const {data:{response}}=await axios.delete(`${API}/notes/${videoId}/${notesIndex}`)
